...
def mail_reminder(mongo):
    remind_mail_since = ""
    mail_settings = mongo.imap_settings.find({"active":True})
    ...
    for mail_setting in mail_settings:
        ...
        if imapObj is not None:
            ...
            reminder_mails = mongo.recruit_mail.find({"is_reminder":True},{"_id":0,"to":1}) # fetching mails from db which should be remind that input we took at the time mail_send remind true or false
            ...
            for ununswered_mail in ununswered_mails:
                ...
                if mail_to:
                    ...
                    if mails in all_mails_should_remind:  #In this condition checking if unanswered mail is of a candidate or anyone else 
                        reminder_mails = mongo.recruit_mail.find_one({"is_reminder":True,"to":mails,"subject":mail_subject})
...
def bounced_mail(mongo):
    print("----bounced running-----")
    mail_settings = mongo.mail_settings.find({"origin": "CAMPAIGN"})
    mail_settings = [serialize_doc(doc) for doc in mail_settings]
    for mail_setting in mail_settings:
        try:
            mail_username = mail_setting['mail_username']
