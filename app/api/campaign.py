...
def create_campaign():
    ...
    if request.method == "POST":
        ...
        if message is not None and message_subject is not None:
            ...
            message_creation.update(
                {"message_id": message_id, "message": message, "message_subject": message_subject, "count": 0})
...
def add_user_campaign():
    ...
    if request.method == "POST":
        ...
        for data in users:
            ...
            data['block'] = False
            unsub_status = mongo.db.unsubscribed_users.find_one(
                {"email": data['email']})
        ...
        mongo.db.campaign_users.create_index(
            [("email", 1), ("campaign", 1)], unique=True)
        try:
...
def delete_user_campaign(campaign_id, user_id):
    ret = mongo.db.campaign_users.remove(
        {"_id": ObjectId(user_id), "campaign": campaign_id})
    vet = mongo.db.mail_status.remove(
        {"user_id": user_id, "campaign": campaign_id})
...
def campaign_start_mail(campaign):
    ...
    if smtps:
        for smtp in smtps:
            smtp_values = mongo.db.mail_settings.find_one(
                {"_id": ObjectId(smtp)})
            try:
                validate = validate_smtp(username=smtp_values['mail_username'], password=smtp_values[
                                         'mail_password'], port=smtp_values['mail_port'], smtp=smtp_values['mail_server'])
                ...
                for data in ids:
                    unsub_detail = mongo.db.campaign_users.find_one(
                        {"_id": ObjectId(data)})
                ...
                smtp_count_value = []
                for smtp in smtps:
                    smtp_detail = mongo.db.mail_settings.find_one(
                        {"_id": ObjectId(smtp)})
                ...
                total_time = (float(len(final_ids)) * delay /
                              float(len(smtp_count_value)))
                if total_time <= 60:
...
def unsub():
    limit = request.args.get('limit', default=0, type=int)
    skip = request.args.get('skip', default=0, type=int)
    ret = mongo.db.unsubscribed_users.find({}).sort(
        'unsubscribe_at', -1).skip(skip).limit(limit)
    ret = [serialize_doc(doc) for doc in ret]
    totalUnsub = 0
    if ret:
        totalUnsub = len(ret)
