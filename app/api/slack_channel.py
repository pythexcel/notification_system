...
from app.slack_util import slack_message,slack_load_token,slack_id,slack_profile,slack_user_load_token
...
def slack():
    ...
    token = slack_load_token()
    sc = SlackClient(token)
    usertoken = slack_user_load_token()
    usersc = SlackClient(usertoken)
    ...
    if request.method == "POST":
        ...
        #getting user private channels into those channels a user exists.
        sl_user_pvt = usersc.api_call(
            "users.conversations",
            types = "private_channel",
            user = slack,
            exclude_archived=True
        ...
        #getting bot private channels into bot exists
        bot_added_in_channels = sc.api_call(
            "users.conversations",
            types = "private_channel",
            exclude_archived=True
        )

        #collecting bot channels into a array
        botchannels =[]            
        botelement = bot_added_in_channels['channels']
        for botdetails in botelement:
            botchannels.append({'value': botdetails['id'], 'text': botdetails['name']})
        ...
        #collecting common channels between bot and a user.
        result = []
        for elem in inner:
            notSame = True
            for dec in botchannels:
                if ((elem["text"] == dec["text"]) and (elem["value"] == dec["value"])):
